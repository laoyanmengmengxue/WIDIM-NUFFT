...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "results_gt_widim_track_nufft_spatialgate_signpass2")
...
def make_rankine(seed=42, Gamma=1500.0, R_core=50.0, sigma_s=SIGMA_S):
    ...
    label = f"Rankine (Gamma={int(Gamma)}, R={int(R_core)} px)"
...
def make_lamb_oseen(seed=42, Gamma=1500.0, sigma_core=40.0, sigma_s=SIGMA_S):
    ...
    label = f"Lamb-Oseen (Gamma={int(Gamma)}, sigma={int(sigma_core)} px)"
    return I1, I2, u_f, v_f, omega_true, label
...
def make_solid_rotation(seed=42, alpha_deg=5.0, sigma_s=SIGMA_S):
    ...
    u_f = np.zeros((FS, FS))
    v_f = np.zeros((FS, FS))
    label = f"Solid Rotation (alpha={alpha_deg:.1f} deg)"
    return I1, I2, u_f, v_f, omega_true, label
...
def _shift_window_translation(I2, cr, cc, half, H, W_img, u, v):
...
def _spatial_snr_gate(alpha_map, snr_map,
                      snr_min=1.8, z_thr=2.8, grad_thr=2.8, lap_thr=2.8,
                      edge_margin=1, edge_delta=0.6):
    ...
    for i in range(n1):
        for j in range(n2):
            if not valid[i, j]:
                continue
            r0 = max(0, i - 1); r1 = min(n1, i + 2)
            c0 = max(0, j - 1); c1 = min(n2, j + 2)
...
def _refill_bad_by_trimmed_mean(alpha_map, pass_mask, n_iter=3, radius=1):
        ...
        for i in range(n1):
            for j in range(n2):
                if (not target[i, j]) or np.isfinite(work[i, j]):
                    continue
                r0 = max(0, i - radius); r1 = min(n1, i + radius + 1)
                c0 = max(0, j - radius); c1 = min(n2, j + radius + 1)
...
def _refill_bad_by_mean(alpha_map, pass_mask, n_iter=2, radius=1):
...
def _second_pass_sign_bad(alpha_map, z_thr=2.5, sign_ratio_thr=0.2):
...
def _two_pass_gate_refill(alpha_map, snr_map, edge_margin=2):
    ...
    fill2 = _refill_bad_by_trimmed_mean(fill1, pass2, n_iter=2, radius=1)
    return fill2


def _eval_one_seed(flow_fn, seed, step_eval=STEP):

    I1, I2, u_f, v_f, omega_field, _ = flow_fn(seed=seed)
    H, W_img = I1.shape
    half = WS // 2

    centers, n, u_wdm, v_wdm = _run_widim(I1, I2, step=step_eval)
    du_dx, du_dy, dv_dx, dv_dy = _compute_gradients(u_wdm, v_wdm, step_eval)
    omega_widim_map = np.degrees(0.5 * (dv_dx - du_dy))

    alpha_map = np.full((n, n), np.nan)
    ...
    for ri, cr in enumerate(centers):
        for ci, cc in enumerate(centers):
            ...
            W2 = _shift_window_translation(I2, cr, cc, half, H, W_img,
                                           u_wdm[ri, ci], v_wdm[ri, ci])
            ...
            alpha_map[ri, ci] = -res["angle_est"]

    alpha_filled = _two_pass_gate_refill(alpha_map, snr_map)
    ...
    for ri, cr in enumerate(centers):
        for ci, cc in enumerate(centers):
            if not np.isfinite(alpha_filled[ri, ci]):
                continue
            omega_nufft_list.append(alpha_filled[ri, ci])
            omega_widim_list.append(omega_widim_map[ri, ci])
            omega_true_list.append(omega_field[cr, cc])
...
def run_mc(flow_fn, n_seeds=N_SEEDS, label="", step_eval=STEP):

    print(f"  [{label}] running {n_seeds} seeds ...", flush=True)
    all_nufft = [];  all_widim = []

    for s in range(n_seeds):
        result = _eval_one_seed(flow_fn, seed=s, step_eval=step_eval)
        if result is None:
            continue
    ...
    print(f"    N={N}  NUFFT RMSE={rmse_n:.3f} [{ci_n[0]:.3f},{ci_n[1]:.3f}]  "
          ...
          f"={rmse_w/max(rmse_n,1e-6):.2f}", flush=True)
...
def _viz_one_scene(ax_true, ax_nufft, ax_widim, ax_err_n, ax_err_w,
        ...
        for ci, cc in enumerate(centers):
            ...
            with contextlib.redirect_stdout(_io.StringIO()):
                res = estimate_rotation(W1, W2, OPT_R_MIN, OPT_WEIGHT,
                                        OPT_POC, verbose=False)
            snr_map[ri, ci] = res["snr"]
            omega_nufft_map[ri, ci] = -res["angle_est"]
            omega_true_map[ri, ci] = omega_field[cr, cc]

    omega_nufft_map = _two_pass_gate_refill(omega_nufft_map, snr_map)
    ...
    im_v = ax_true.imshow(omega_true_map, **kw)
    ax_true.set_title(r"Ground Truth $\Omega$", fontsize=8)
    ax_nufft.imshow(omega_nufft_map, **kw)
    ax_nufft.set_title(r"WIDIM-track+NUFFT $\hat{\Omega}$ (Spatial Gate)", fontsize=8)
    ax_widim.imshow(omega_widim_map, **kw)
    ax_widim.set_title(r"WIDIM Gradient $\hat{\Omega}$", fontsize=8)
    ...
    ax_err_n.set_title(rf"NUFFT Abs Error" + "\n" + rf"(RMSE={rmse_n:.2f}$^\circ$)", fontsize=7)
    ax_err_w.set_title(rf"WIDIM Abs Error" + "\n" + rf"(RMSE={rmse_w:.2f}$^\circ$)", fontsize=7)
...
def plot_main(results, viz_fns, seed_viz=42, step_viz=10):
    ...
    fig.suptitle(
        "Ground-Truth Vorticity Comparison: NUFFT vs WIDIM Gradient\n"
        + rf"($\sigma_s$={SIGMA_S}, WS={WS} px, MC N={N_SEEDS} seeds)",
        fontsize=10,
        fontweight='bold',
    )
    ...
    for row, (fn, label) in enumerate(viz_fns):
        ...
        cbar1.set_label(r"Vorticity ($^\circ$/frame)", fontsize=6)
        cbar2 = fig.colorbar(im_en, cax=cb2)
        cbar2.ax.tick_params(labelsize=6)
        cbar2.set_label(r"Abs Error ($^\circ$/frame)", fontsize=6)
...
def plot_summary(results_list):
    ...
    ax.set_title("Ground-Truth Vorticity RMSE: NUFFT vs WIDIM Gradient\n"
                 f"(95% Bootstrap CI, N={N_SEEDS} seeds each, _s={SIGMA_S})",
                 fontsize=10)
...
def generate_fig1_motivation(seed=42, out_name="fig01_motivation.png", step_fig=10):
    ...
    vmax = np.nanmax(np.abs(omega_true_map)) * 1.2 + 0.1
    diff = np.where(np.isfinite(omega_nufft_map), omega_widim_map - omega_nufft_map, np.nan)
    err_abs = np.abs(diff)
    ...
    im1 = axes[1].imshow(omega_nufft_map, cmap=cmap_v,
                         vmin=-vmax, vmax=vmax, aspect="auto")
    axes[1].set_title("WIDIM-track+NUFFT Vorticity (Spatial Gate)", fontsize=10)
...
def save_results_txt(results_list, out_path):
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("=" * 72 + "\n")
        f.write("Ground-Truth Vorticity Comparison Results (WIDIM-track+NUFFT, SPATIAL GATE)\n")
...
def main():
    ...
    print("=" * 60)
    print("Ground-Truth Vorticity Comparison (WIDIM-track+NUFFT, SPATIAL GATE)")
    print(f"sigma_s={SIGMA_S}, WS={WS}px, N_SEEDS={N_SEEDS}, STEP={UNIFIED_STEP}")
    print("=" * 60)

    print("\n[Step 0] Generate Fig.1 motivation ...")
    generate_fig1_motivation(seed=42, step_fig=UNIFIED_STEP)
    ...
    for fn, label in flow_specs:
        r = run_mc(fn, n_seeds=N_SEEDS, label=label, step_eval=UNIFIED_STEP)
        results_list.append(r)

    print("\n[Step 2] eed=42?..")
    viz_fns = [(fn, label) for fn, label in flow_specs]
    plot_main(results_list, viz_fns, seed_viz=42, step_viz=UNIFIED_STEP)
