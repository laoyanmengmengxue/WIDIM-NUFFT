...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "results_gt_widim_track_nufft_spatialgate_edgepass")
...
def _two_pass_gate_refill(alpha_map, snr_map, edge_margin=2):
    ...
    gate2_all = _spatial_snr_gate(fill1, snr_map)

    n1, n2 = fill1.shape
    edge = np.zeros((n1, n2), dtype=bool)
    ...
    gate2 = gate2_all.copy()
    gate2[~edge] = True

    fill2 = _refill_bad_by_mean(fill1, gate2, n_iter=2, radius=1)
    return fill2
...
def _viz_one_scene(ax_true, ax_nufft, ax_widim, ax_err_n, ax_err_w,
    ...
    im_v = ax_true.imshow(omega_true_map, **kw); ax_true.set_title("Ground Truth ?", fontsize=8)
    ax_nufft.imshow(omega_nufft_map, **kw); ax_nufft.set_title("WIDIM-track+NUFFT ? (Spatial Gate)", fontsize=8)
    ax_widim.imshow(omega_widim_map, **kw); ax_widim.set_title("WIDIM Gradient ?", fontsize=8)
