...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "results_gt_widim_nufft_nogate")
...
def _eval_one_seed(flow_fn, seed):
    ...
    for ri, cr in enumerate(centers):
        for ci, cc in enumerate(centers):
            ...
            W2 = _deform_window(I2, cr, cc, half, H, W_img,
                                u_wdm[ri, ci], v_wdm[ri, ci],
                                du_dx[ri, ci], du_dy[ri, ci],
                                dv_dx[ri, ci], dv_dy[ri, ci])
...
def generate_fig1_motivation(seed=42, out_name="fig01_motivation.png", step_fig=10):
    ...
    im1 = axes[1].imshow(omega_nufft_map, cmap=cmap_v,
                         vmin=-vmax, vmax=vmax, aspect="auto")
    axes[1].set_title("WIDIM+NUFFT Vorticity (No Gate)", fontsize=10)
...
def save_results_txt(results_list, out_path):
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("=" * 72 + "\n")
        f.write("Ground-Truth Vorticity Comparison Results (WIDIM+NUFFT, NO GATE)\n")
...
def main():
    ...
    print("=" * 60)
    print("Ground-Truth Vorticity Comparison (WIDIM+NUFFT, NO GATE)")
